...
ggplot(df_limma, x =-log10(msi_qval), aes(x = beta_msi, y =-log10(msi_qval), color = is_sig_graph)) + 
  ...
  scale_color_brewer(palette = "Set2", name = "Interaction") +
...
```{r large-plot}
lsize <- 11
bottom_left <- plot_grid(goplot, 
                         plot_grid(NULL, limma_plot, nrow = 1, rel_widths = c(1,8)), 
                         ncol = 1, labels = c("B", "C"),
                         label_size = lsize)
bottom_grid <- plot_grid(bottom_left, gene_plot, nrow = 1, labels = c("", "D"),
                         label_size = lsize)

plot_grid(lplot, bottom_grid, ncol = 1, rel_heights = c(2,1.5), labels = c("A",""),
          label_size = lsize)
```
